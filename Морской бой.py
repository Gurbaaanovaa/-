...
def button_begin_again():
    global list_ids, enemy_ships
    for el in list_ids:
        canvas.delete(el)
    list_ids = []
    generate_enemy_ships()  # Re-generate enemy ships
...
def add_to_all(event):
    _type = 0  # Left Click
    if event.num == 3:
        _type = 1  # Right Click (not used currently)
    ...
    # Check if the cell clicked has part of a ship
    if enemy_ships[ip_y][ip_x] > 0:  # If there's a ship
        # Create a rectangle to indicate a hit
        _id = canvas.create_rectangle(ip_x * step_x, ip_y * step_y,
                                      ip_x * step_x + step_x, ip_y * step_y + step_y, fill="red")
        list_ids.append(_id)
        print("Hit at: ({}, {})".format(ip_x, ip_y))
    else:
        # Indicate a miss
        _id = canvas.create_rectangle(ip_x * step_x, ip_y * step_y,
                                      ip_x * step_x + step_x, ip_y * step_y + step_y, fill="blue")
        list_ids.append(_id)
        print("Miss at: ({}, {})".format(ip_x, ip_y))
